...
//test
	...
	$ch = curl_init($api);
	curl-setopt($ch, CURLOPT_POST, true);
	curl-setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
	curl-setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl-setopt($ch, CURLOPT_POSTFIELDS, json_encode($body));
	curl-setopt($ch, CURLOPT_HTTPHEADER, array(
    ...
    public function replyImage($url){
	$api = $this-> apiReply;
	$webhook = $this ->webhookEventObject;
	$ReplyToken = $webhook-> {"events"}[0]->{"replyToken"};
	$body["replyToken"] = $replyToken;
	$body ["message"][0] = array(
			"type" => "image",
